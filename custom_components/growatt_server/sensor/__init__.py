...
from .sph_classic import SPH_CLASSIC_SENSOR_TYPES
...
async def async_setup_entry(
    ...
    for device_sn, device_coordinator in data.devices.items():
        ...
        if device_coordinator.device_type == "inverter":
            ...
            # Classic-mode SPH uses newTwoSphAPI.do with a different
            # response shape than V1's sph_detail/sph_energy.
            if device_coordinator.api_version == "v1":
                sensor_descriptions = list(SPH_SENSOR_TYPES)
            else:
                sensor_descriptions = list(SPH_CLASSIC_SENSOR_TYPES)
        else:
            _LOGGER.debug(
